...
| [Python 3](https://www.python.org/) | 3.9+ | two helper scripts run during every build (`convert_spec.py`) and for checks (`tools/gio_check.py`) |
| Python package `pyyaml` | any | `pip install pyyaml` — used by both scripts |
| Git | any | optional, but "last updated" dates on pages and in the sitemap come from git history |

Check: `node --version`, `python --version`, `python -c "import yaml"`.
...
`npm run check` (or `check:salesplay`, `check:vendrex`, `check:sellmo`) runs `tools/gio_check.py` on each finished build and **fails with one line per problem** if any file in a brand's output mentions another brand, a machine-readable file is missing, a link in `llms.txt` points nowhere, or a page has no `description`, more than one H1, or an emoji heading. Run it before uploading.

### What a build does

1. Reads `.env.<brand>` (via `tenant.js`) — no `TENANT` set means SalesPlay.
2. Runs `python convert_spec.py` to write `static/api_spec.json` with the brand's API host.
...
| `python gen_api_docs.py` | regenerate the 53 endpoint pages from `api_spec.yaml` — never edit those pages by hand |
| `python convert_spec.py` | write `static/api_spec.json` for the current brand (runs automatically on build/start) |
...
gen_api_docs.py             api_spec.yaml → docs/API-reference/<group>/*.md
convert_spec.py             api_spec.yaml → static/api_spec.json (per brand)
...
tools/gio_check.py          the post-build checker behind `npm run check`
...
5. Don't hand-edit `docs/API-reference/<group>/*.md` — change `api_spec.yaml` and run `python gen_api_docs.py`.
...
| `python: command not found` / `No module named yaml` | Install Python 3 and `pip install pyyaml` |
