...
sidebar_label: Delete a Single Webhook
sidebar_position: 7
description: "DELETE /webhooks — remove a webhook subscription from a SalesPlay account by its ID."
---
...
**DELETE /webhooks** — remove a webhook subscription from a SalesPlay account by its ID. Send parameters as a JSON request body (`id`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--delete">DELETE</span><code>https://api.salesplaypos.com/v1.0/webhooks</code></div>
...
|---|---|---|
| `id` <span className="api-required">required</span> | string | The unique identifier of the webhook to delete. |
...
```bash
curl -X DELETE "https://api.salesplaypos.com/v1.0/webhooks" \
...
const req = https.request("https://api.salesplaypos.com/v1.0/webhooks", {
  method: "DELETE",
  headers: {
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"id": "string"}));
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("DELETE"), "https://api.salesplaypos.com/v1.0/webhooks");
...
| Name | Type | Description |
|---|---|---|
| `deleted_object_ids` | string | ID of the deleted webhook |
