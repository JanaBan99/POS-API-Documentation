...
sidebar_label: Delete Order Type
sidebar_position: 3
description: "DELETE /order_types — delete an order type from a SalesPlay account by its ID."
---
...
**DELETE /order_types** — delete an order type from a SalesPlay account by its ID. Send parameters as a JSON request body (`order_type_id`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--delete">DELETE</span><code>https://api.salesplaypos.com/v1.0/order_types</code></div>
...
This endpoint reads its parameters from the JSON request body — even on `DELETE`. Query-string parameters are ignored.
:::

| Name | Type | Description |
|---|---|---|
| `order_type_id` <span className="api-required">required</span> | string | Unique identifier of the order type to be deleted. |
...
```bash
curl -X DELETE "https://api.salesplaypos.com/v1.0/order_types" \
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"order_type_id": "string"}));
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("DELETE"), "https://api.salesplaypos.com/v1.0/order_types");
...
**200** — Order type deleted successfully
...
| `order_type_id` | string | Unique identifier of the deleted order type. |
