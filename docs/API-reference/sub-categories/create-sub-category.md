---
title: Create or Update Sub Category
sidebar_label: Create or Update Sub Category
sidebar_position: 2
description: "POST /sub_category — create a sub category under a category in SalesPlay, or update it when the code already exists."
---
...
**POST /sub_category** — create a sub category under a category in SalesPlay, or update it when the code already exists. Send parameters as a JSON request body (`category_id`, `sub_category_name`, `created_at`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--post">POST</span><code>https://api.salesplaypos.com/v1.0/sub_category</code></div>
...
|---|---|---|
| `category_id` <span className="api-required">required</span> | string | The ID of the parent category |
| `sub_category_name` <span className="api-required">required</span> | string | The name of the subcategory |
| `created_at` <span className="api-required">required</span> | string (date-time) | The creation date of the subcategory (Y-m-d H:i:s) 24 hours format |
...
```bash
curl -X POST "https://api.salesplaypos.com/v1.0/sub_category" \
...
```javascript
const res = await fetch("https://api.salesplaypos.com/v1.0/sub_category", {
  method: "POST",
  headers: {
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"category_id": "string", "sub_category_name": "string", "created_at": "2025-01-15 10:30:00"}));
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("POST"), "https://api.salesplaypos.com/v1.0/sub_category");
...
**200** — Subcategory created successfully
