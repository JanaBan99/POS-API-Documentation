...
sidebar_position: 3
description: "DELETE /sub_category — delete a sub category from a SalesPlay account by its ID."
---
...
**DELETE /sub_category** — delete a sub category from a SalesPlay account by its ID. Send parameters as a JSON request body (`category_id`, `sub_category_id`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--delete">DELETE</span><code>https://api.salesplaypos.com/v1.0/sub_category</code></div>
...
| `sub_category_id` <span className="api-required">required</span> | string | The ID of the sub category |
...
```bash
curl -X DELETE "https://api.salesplaypos.com/v1.0/sub_category" \
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"category_id": "string", "sub_category_id": "string"}));
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("DELETE"), "https://api.salesplaypos.com/v1.0/sub_category");
...
**200** — Sub Category deleted successfully

```json
{
  "success": {
    "code": "SUCCESS",
    "details": "Sub Category successfully deleted."
  }
}
```
...
| Name | Type | Description |
|---|---|---|
| `code` | string | 200 Example: `SUCCESS` |
| `details` | string | Sub Category successfully deleted. Example: `Sub Category successfully deleted.` |
