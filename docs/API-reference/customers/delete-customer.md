...
sidebar_position: 3
description: "DELETE /customers — delete a customer profile from a SalesPlay account by its ID."
---
...
**DELETE /customers** — delete a customer profile from a SalesPlay account by its ID. Send parameters as a JSON request body (`customer_id`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--delete">DELETE</span><code>https://api.salesplaypos.com/v1.0/customers</code></div>
...
|---|---|---|
| `customer_id` <span className="api-required">required</span> | string | Unique identifier of the customer to be deleted. |
...
```bash
curl -X DELETE "https://api.salesplaypos.com/v1.0/customers" \
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"customer_id": "string"}));
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("DELETE"), "https://api.salesplaypos.com/v1.0/customers");
