...
sidebar_label: Get Measurements
sidebar_position: 1
description: "GET /measurements — list the units of measurement (kg, pcs, litre) in a SalesPlay account, paginated with a cursor."
---
...
**GET /measurements** — list the units of measurement (kg, pcs, litre) in a SalesPlay account, paginated with a cursor. Send parameters as a JSON request body (`measurement_ids`, `created_at_min`, `created_at_max`, `limit`, `cursor`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--get">GET</span><code>https://api.salesplaypos.com/v1.0/measurements</code></div>
...
|---|---|---|
| `measurement_ids` | string | Filter by measurement IDs |
...
| `limit` | integer | Number of measurements to return Default: `10` |
...
```bash
curl -X GET "https://api.salesplaypos.com/v1.0/measurements" \
...
const req = https.request("https://api.salesplaypos.com/v1.0/measurements", {
  method: "GET",
  headers: {
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"measurement_ids": "string", "created_at_min": "2025-01-15 10:30:00", "created_at_max": "2025-01-15 10:30:00", "limit": 10, "cursor": "string"}));
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("GET"), "https://api.salesplaypos.com/v1.0/measurements");
...
<details>
<summary><code>measurements[]</code> — child attributes</summary>
...
| `updated_date` | string (date-time) | The last updated date of the measurement (Y-m-d H:i:s) 24 hours format |
