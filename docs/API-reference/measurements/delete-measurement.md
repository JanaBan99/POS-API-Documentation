...
sidebar_label: Delete Measurement
sidebar_position: 3
description: "DELETE /measurements — delete a unit of measurement from a SalesPlay account by its ID."
---
...
**DELETE /measurements** — delete a unit of measurement from a SalesPlay account by its ID. Send parameters as a JSON request body (`measurement_id`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--delete">DELETE</span><code>https://api.salesplaypos.com/v1.0/measurements</code></div>
...
|---|---|---|
| `measurement_id` <span className="api-required">required</span> | string | The measurement ID to delete |
...
```bash
curl -X DELETE "https://api.salesplaypos.com/v1.0/measurements" \
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"measurement_id": "string"}));
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("DELETE"), "https://api.salesplaypos.com/v1.0/measurements");
...
```json
{
  "success": {
    "code": "SUCCESS",
    "details": "Measurement deleted successfully."
  }
}
```
...
| Name | Type | Description |
|---|---|---|
| `code` | string | SUCCESS Example: `SUCCESS` |
| `details` | string | Measurement deleted successfully. Example: `Measurement deleted successfully.` |
