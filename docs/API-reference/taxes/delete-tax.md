---
title: Delete Tax
sidebar_label: Delete Tax
sidebar_position: 3
description: "DELETE /taxes — delete a tax rate from a SalesPlay account by its ID."
---
...
**DELETE /taxes** — delete a tax rate from a SalesPlay account by its ID. Send parameters as a JSON request body (`tax_code`, `shops`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--delete">DELETE</span><code>https://api.salesplaypos.com/v1.0/taxes</code></div>
...
|---|---|---|
| `tax_code` <span className="api-required">required</span> | string | Unique tax code to be deleted Example: `TC750` |
| `shops` <span className="api-required">required</span> | array[string] | List of shop IDs where the tax should be deleted |
...
```bash
curl -X DELETE "https://api.salesplaypos.com/v1.0/taxes" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
  "tax_code": "TC750",
  "shops": [
    "UjFEdGgwbmU1eDdRWkJVdDd1ZWRSUT09"
  ]
}'
```
...
const req = https.request("https://api.salesplaypos.com/v1.0/taxes", {
  method: "DELETE",
  headers: {
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"tax_code": "TC750", "shops": ["UjFEdGgwbmU1eDdRWkJVdDd1ZWRSUT09"]}));
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("DELETE"), "https://api.salesplaypos.com/v1.0/taxes");
...
| Name | Type | Description |
|---|---|---|
| `code` | string | Example: `SUCCESS` |
| `details` | string | Example: `successfully deleted.` |
