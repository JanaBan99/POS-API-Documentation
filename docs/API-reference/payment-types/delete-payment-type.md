...
sidebar_label: Delete Payment Type
sidebar_position: 3
description: "DELETE /payment_types — delete a custom payment method from a SalesPlay account by its ID (default types cannot be deleted)."
---
...
**DELETE /payment_types** — delete a custom payment method from a SalesPlay account by its ID (default types cannot be deleted). Send parameters as a JSON request body (`payment_type_id`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--delete">DELETE</span><code>https://api.salesplaypos.com/v1.0/payment_types</code></div>
...
|---|---|---|
| `payment_type_id` <span className="api-required">required</span> | string | Unique identifier of the payment type to be deleted. |
...
```bash
curl -X DELETE "https://api.salesplaypos.com/v1.0/payment_types" \
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"payment_type_id": "string"}));
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("DELETE"), "https://api.salesplaypos.com/v1.0/payment_types");
...
**200** — Payment type deleted successfully
...
| `payment_type_code` | string | Code of the deleted payment type. |
| `payment_type_id` | string | Unique identifier of the deleted payment type. |
