...
sidebar_position: 1
description: "GET /suppliers — list the suppliers in a SalesPlay account, filtered by ID or date, paginated with a cursor."
---
...
**GET /suppliers** — list the suppliers in a SalesPlay account, filtered by ID or date, paginated with a cursor. Send parameters as a JSON request body (`supplier_ids`, `updated_at_min`, `updated_at_max`, `limit`, `cursor`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--get">GET</span><code>https://api.salesplaypos.com/v1.0/suppliers</code></div>
...
|---|---|---|
| `supplier_ids` | string | Filter by supplier IDs |
| `updated_at_min` | string (date-time) | Minimum update date (Y-m-d H:i:s) 24 hours format |
| `updated_at_max` | string (date-time) | Maximum update date (Y-m-d H:i:s) 24 hours format |
| `limit` | integer | Number of suppliers to return Default: `10` |
...
```bash
curl -X GET "https://api.salesplaypos.com/v1.0/suppliers" \
...
const req = https.request("https://api.salesplaypos.com/v1.0/suppliers", {
  method: "GET",
  headers: {
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"supplier_ids": "string", "updated_at_min": "2025-01-15 10:30:00", "updated_at_max": "2025-01-15 10:30:00", "limit": 10, "cursor": "string"}));
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("GET"), "https://api.salesplaypos.com/v1.0/suppliers");
...
<details>
<summary><code>suppliers[]</code> — child attributes</summary>
