...
sidebar_position: 3
description: "DELETE /suppliers — delete a supplier from a SalesPlay account by its ID."
---
...
**DELETE /suppliers** — delete a supplier from a SalesPlay account by its ID. Send parameters as a JSON request body (`supplier_id`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--delete">DELETE</span><code>https://api.salesplaypos.com/v1.0/suppliers</code></div>
...
|---|---|---|
| `supplier_id` <span className="api-required">required</span> | string | The supplier ID to delete |
...
```bash
curl -X DELETE "https://api.salesplaypos.com/v1.0/suppliers" \
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"supplier_id": "string"}));
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("DELETE"), "https://api.salesplaypos.com/v1.0/suppliers");
...
| `details` | string | Supplier deleted successfully. Example: `Supplier deleted successfully.` |
