---
title: Get Pay-Ins and Pay-Outs
sidebar_label: Get Pay-Ins and Pay-Outs
sidebar_position: 2
description: "GET /drawer_transaction — list the cash drawer pay-ins and pay-outs recorded on SalesPlay POS terminals during shifts (read-only)."
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Get Pay-Ins and Pay-Outs

**GET /drawer_transaction** — list the cash drawer pay-ins and pay-outs recorded on SalesPlay POS terminals during shifts (read-only). Send parameters as a JSON request body (`shop_ids`, `pos_device_ids`, `shift_ids`, `created_at_min`, `created_at_max`, `limit`, `cursor`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--get">GET</span><code>https://api.salesplaypos.com/v1.0/drawer_transaction</code></div>
...
| `shift_ids` | string | List of shift IDs (comma-separated) |
...
```bash
curl -X GET "https://api.salesplaypos.com/v1.0/drawer_transaction" \
...
const req = https.request("https://api.salesplaypos.com/v1.0/drawer_transaction", {
  method: "GET",
  headers: {
...
url = "https://api.salesplaypos.com/v1.0/drawer_transaction"
...
```php
<?php
$ch = curl_init("https://api.salesplaypos.com/v1.0/drawer_transaction");
curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode({"shop_ids": "string", "pos_device_ids": "string", "shift_ids": "string", "created_at_min": "2025-01-15 10:30:00", "created_at_max": "2025-01-15 10:30:00", "limit": 10, "cursor": "string"}));
...
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("https://api.salesplaypos.com/v1.0/drawer_transaction"))
...
```csharp
var request = new HttpRequestMessage(new HttpMethod("GET"), "https://api.salesplaypos.com/v1.0/drawer_transaction");
  ...
  "transactions": [
    {
      ...
      "shift_id": "string",
      "pos_device_transaction_id": "string",
...
<details>
<summary><code>transactions[]</code> — child attributes</summary>

| Name | Type | Description |
|---|---|---|
| `shop_id` | string | Shop ID |
| `pos_device_id` | string | POS device ID |
| `shift_id` | string | Unique identifier of the shift |
| `pos_device_transaction_id` | string | Unique identifier of the POS device transaction |
| `transaction_type` | string<br/>enum: `Credit` \| `Debit` | Transaction type |
| `transaction_amount` | string | Transaction amount |
| `transaction_reason` | string | Reason for the transaction |
| `other_description` | string | Additional description for the transaction |
| `cashier_username` | string | Username of the cashier who performed the transaction |
| `transaction_date` | string (date-time) | The time when this transaction was created |
