...
sidebar_position: 1
description: "GET /category — list the product categories in a SalesPlay account, filtered by ID or date, paginated with a cursor."
---
...
**GET /category** — list the product categories in a SalesPlay account, filtered by ID or date, paginated with a cursor. Send parameters as a JSON request body (`category_ids`, `created_at_min`, `created_at_max`, `limit`, `cursor`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--get">GET</span><code>https://api.salesplaypos.com/v1.0/category</code></div>
...
|---|---|---|
| `category_ids` | string | Return only categories specified by a comma-separated list of IDs |
...
| `limit` | integer | Number of categories to return Default: `10` |
...
<details>
<summary><code>categories[]</code> — child attributes</summary>
...
| `updated_date` | string (date-time) | The last updated date of the category (Y-m-d H:i:s) 24 hours format |
