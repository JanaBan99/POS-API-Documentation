...
sidebar_position: 3
description: "DELETE /category — delete a product category from a SalesPlay account by its ID."
---
...
**DELETE /category** — delete a product category from a SalesPlay account by its ID. Send parameters as a JSON request body (`category_id`). Requires a Bearer token in the `Authorization` header. Base URL: `https://api.salesplaypos.com/v1.0`.



<div className="api-endpoint"><span className="api-badge api-badge--delete">DELETE</span><code>https://api.salesplaypos.com/v1.0/category</code></div>
...
|---|---|---|
| `category_id` <span className="api-required">required</span> | string | The unique identifier for the category to delete |
...
|---|---|---|
| `code` | string | 200 |
| `details` | string | Category deleted successfully |
| `category_id` | string | The deleted category ID |
